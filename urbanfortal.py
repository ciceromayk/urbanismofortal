...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("🗺️ Opções de Mapa")
    tile_selection = st.selectbox("Selecione a Camada Base:", list(MAP_TILES.keys()))
...
def reverse_geocode(lat, lon):
    """Converte coordenadas em endereço usando Nominatim."""
    try:
        url = f"https://nominatim.openstreetmap.org/reverse?lat={lat}&lon={lon}&format=json"
        response = requests.get(url, headers={"User-Agent": "UrbanFortalApp/1.0"}, timeout=10)
        data = response.json()
        return data.get("display_name", "Endereço não identificado.")
    except:
        return "Erro ao buscar o endereço."

# --- FUNÇÃO AUXILIAR: EXIBIR INFO NO SIDEBAR ---
def exibir_info_zona(zona_encontrada, endereco_str=None):
    """Exibe informações urbanísticas no sidebar."""
    if not zona_encontrada.empty:
        ...
        with sidebar_placeholder.container():
            ...
            st.markdown(
                f"**Geometria:**<br>Área: **{area_ha:.2f} ha**<br>Perímetro: **{perimetro_m:.0f} m**",
            ...
            params = pd.DataFrame(
                {
                    ...
                    "Valor": [
                        z["indice_aproveitamento_basico"],
                        z["indice_aproveitamento_maximo"],
                        z["taxa_ocupacao_solo"],
                        z["taxa_ocupacao_subsolo"],
                        z["altura_maxima"],
                        z["taxa_permeabilidade"],
                    ],
                }
            ).set_index("Parâmetro")
            st.dataframe(params)
...
if coord_busca and zona_geojson:
    ...
    folium.CircleMarker(
        ...
        fill_color="yellow",
        fill_opacity=0.9,
        popup=f"Endereço Buscado:<br>{info_zona_busca['nome_zona']}",
    ).add_to(m)
...
st.markdown("**🖱️ Clique em qualquer ponto para identificar a zona correspondente.**")

map_data = st_folium(m, height=700, width=None, returned_objects=["last_clicked"], key="mapa_principal")
...
if map_data and map_data.get("last_clicked"):
    ...
    if not zona_ponto_clicado.empty:
        ...
        info_zona_clicada, geojson_clicado = exibir_info_zona(zona_ponto_clicado, endereco_str=endereco_completo)
        ...
        folium.GeoJson(
            ...
            tooltip=info_zona_clicada["nome_zona"],
        ).add_to(m)
        ...
        st_folium(m, height=700, width=None, key="mapa_atualizado")
